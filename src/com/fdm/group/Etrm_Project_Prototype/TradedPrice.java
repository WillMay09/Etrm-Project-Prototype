 ...
 *
 * This is a historical fact — it never changes after the trade. It is separate
 * from TradeInfo because: - Options: the price is the premium paid (e.g.
 * $3.50/barrel) - Futures: the price is the entry price locked in (e.g. $78.50)
 * - Physical: the price is the agreed forward price
 *
 * It is separate from current market price, which changes constantly.
 *
 * OpenGamma source: TradedPrice holds tradeDate + price. We extend this
 * slightly for ETRM by adding currency and priceUnit.
 *
 * Note: Position does NOT have a TradedPrice because a position aggregates
 * multiple trades at different prices — there is no single agreed price at the
 * position level.
 */
